-- ============================================================
-- INITIALIZE MICROSERVICES DATABASES & BOOKING SEED DATA
-- Executed automatically on first PostgreSQL container start
-- ============================================================

-- 1. CREATE ALL MICROSERVICES DATABASES IF NOT EXIST
...
SELECT 'CREATE DATABASE cinema_db' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'cinema_db')\gexec
SELECT 'CREATE DATABASE showtime_db' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'showtime_db')\gexec
SELECT 'CREATE DATABASE booking_db' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'booking_db')\gexec

-- 2. CONNECT TO BOOKING DATABASE
\c booking_db;

-- 3. CREATE BOOKING SERVICE TABLES
...
-- 4. CLEAN OLD DATA IF RE-RUNNING
...
-- 5. INSERT SEED DATA FOR BOOKING SERVICE

-- 5.1 Booking Settings
