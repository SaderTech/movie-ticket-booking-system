...
import { AlertCircle, Clock3, CreditCard, ShieldCheck, TestTube2, Ticket } from 'lucide-react'
...
export function CheckoutPage() {
  ...
  const returnToSeats = async () => {
    await releaseHold()
    navigate(`/booking/${checkout.showtime?.id}/seats`)
  }
  ...
  return <div className="booking-page"><div className="container">
    ...
    <div className="checkout-layout"><section className="panel checkout-main"><div className="checkout-movie"><SafeImage src={checkout.movie?.posterUrl} alt={`Poster ${checkout.movie?.title || ''}`} /><div><span className="eyebrow"><Ticket /> Vé của bạn</span><h2>{checkout.movie?.title || `Phim #${checkout.showtime?.movieId}`}</h2><p>{checkout.cinema?.name || checkout.hall?.cinemaName || `Rạp #${checkout.showtime?.cinemaId}`} · {checkout.hall?.name || `Phòng #${checkout.showtime?.roomId}`}</p><strong>{formatDate(checkout.showtime?.showDate)} · {formatTime(checkout.showtime?.startTime)}</strong></div></div><div className="detail-list compact"><div><span>Ghế</span><strong>{checkout.seatCodes?.join(', ')}</strong></div><div><span>Số lượng</span><strong>{checkout.seatCodes?.length} vé</strong></div></div><div><h2>Phương thức thanh toán</h2><p className="muted">Chọn cổng thanh toán được backend hỗ trợ.</p></div><div className="payment-options"><label className={method === 'VNPAY' ? 'selected' : ''}><input type="radio" name="payment" value="VNPAY" checked={method === 'VNPAY'} onChange={(event) => setMethod(event.target.value)} /><span className="payment-icon"><CreditCard /></span><span><strong>VNPay Sandbox</strong><small>Thanh toán qua cổng VNPay và quay lại MovieTicket.</small></span><i /></label>{import.meta.env.DEV && <label className="disabled"><input type="radio" name="payment" value="MOCK" disabled /><span className="payment-icon"><TestTube2 /></span><span><strong>MOCK</strong><small>Backend hiện tại chỉ chấp nhận VNPAY nên lựa chọn này được vô hiệu hóa.</small></span></label>}</div></section><aside className="panel booking-summary payment-summary"><span className="eyebrow">Tóm tắt thanh toán</span><div className="summary-row"><span>{checkout.seatCodes?.length} ghế</span><strong>{formatCurrency(estimate)}</strong></div><div className="summary-total"><span>Tổng tạm tính</span><strong>{formatCurrency(estimate)}</strong></div>{countdown.expired && <p className="error-note">Phiên giữ ghế đã hết hạn. Vui lòng chọn lại.</p>}<button className="button button-primary button-lg full" disabled={countdown.expired || confirm.isPending || method !== 'VNPAY'} onClick={() => confirm.mutate()}>{confirm.isPending ? <><LoadingSpinner /> Đang tạo thanh toán…</> : <><ShieldCheck /> Thanh toán với VNPay</>}</button><button className="button button-secondary full" disabled={confirm.isPending} onClick={returnToSeats}>Quay lại chọn ghế</button></aside></div>
  </div></div>
}
